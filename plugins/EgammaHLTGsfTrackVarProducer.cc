...
EgammaHLTGsfTrackVarProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(reco::ElectronCollection::const_iterator iElectronCand = electronHandle->begin(); iElectronCand != electronHandle->end(); iElectronCand++){
    reco::ElectronRef gsfElectronCandRef(electronHandle,iElectronCand-electronHandle->begin());  
    ...
    for( reco::RecoEcalCandidateCollection::const_iterator iRecoEcalCand = recoEcalCandHandle->begin(); iRecoEcalCand != recoEcalCandHandle->end(); iRecoEcalCand++){
      ...
      const reco::SuperClusterRef scRef = iRecoEcalCand->superCluster();
      ...
      if(iElectronCand->superCluster()==scRef) gsfTracks.push_back(&*iElectronCand->gsfTrack());
    }
    ...
    if(static_cast<int>(gsfTracks.size())>=upperTrackNrToRemoveCut_){
	  ...
	  GlobalPoint scPos(iElectronCand->superCluster()->x(),iElectronCand->superCluster()->y(),iElectronCand->superCluster()->z());
	  GlobalPoint trackExtrapToSC = trackExtrapolator_.extrapolateTrackPosToPoint(*gsfTracks[trkNr],scPos);
	  EleRelPointPair scAtVtx(iElectronCand->superCluster()->position(),trackExtrapToSC,beamSpot.position());
    ...
    dEtaMap.insert(gsfElectronCandRef, dEtaInValue);
    dPhiMap.insert(gsfElectronCandRef, dPhiInValue);
  }

  std::auto_ptr<reco::ElectronIsolationMap> dEtaMapForEvent(new reco::ElectronIsolationMap(dEtaMap));
  std::auto_ptr<reco::ElectronIsolationMap> dPhiMapForEvent(new reco::ElectronIsolationMap(dPhiMap));
