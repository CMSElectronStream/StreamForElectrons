...
void FilterGenParticles::produce (Event & ev, const EventSetup &) {
  ...
  for( size_t i = 0; i < pGenPart->size(); ++ i ) {
        ...
        if ( fabs(genpart.pdgId())!=genLevelSelect_) continue; // check the pdgId -> should be an electron at generator level with status 1 -> coming from ME
        if ( genpart.status()!=genLevelStatus_) continue;
        if ( genpart.pt()<minPtThreshold_) continue; // above a minimum pT threshold and eta acceptance
        if ( fabs(genpart.eta())>maxEtaThreshold_) continue;

        double sumPT=0;
        for( size_t j = 0; j < pGenPart->size(); ++ j ) {
	  const reco::GenParticle& genpart2 = (*pGenPart)[j]; // loop on all the other gen particles to compute isolation at generator level -> no pile-up, only ME + PS particles
                if (i==j) continue;
