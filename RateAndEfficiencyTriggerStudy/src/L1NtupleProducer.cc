...
//gct
  ...
  NonIsoEmSource_      = ps.getParameter<edm::InputTag>("NonIsoEmSource");
...
void L1NtupleProducer::analyzeGCT(const edm::Event& e,const edm::EventSetup& es)
{
  ...
  edm::Handle< std::vector< UCTCandidate > > L1NonIsoEmUCT ;
  e.getByLabel(NonIsoEmSource_, L1NonIsoEmUCT);

  if (!L1NonIsoEmUCT.isValid() && NonIsoEmSource_.label() != "none") 
    edm::LogWarning("DataNotFound") << " Could not find l1NonIsoEmUCT "" elements, label was " << NonIsoEmSource_ ;

    
  if (l1IsoEm.isValid() && l1NonIsoEm.isValid() && L1NonIsoEmUCT.isValid())
    pL1gct->SetEm(l1IsoEm, l1NonIsoEm, L1NonIsoEmUCT,emScale);
  else if (l1IsoEm.isValid() && l1NonIsoEm.isValid())
