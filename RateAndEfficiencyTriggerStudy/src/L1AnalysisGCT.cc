...
void L1Analysis::L1AnalysisGCT::SetEm(const edm::Handle < L1GctEmCandCollection > l1IsoEm,
	   const edm::Handle < L1GctEmCandCollection > l1NonIsoEm,
	   const edm::Handle < std::vector<UCTCandidate> > l1NonIsoEmUCT,
	   edm::ESHandle< L1CaloEtScale > emScal){
	...
	gct_.NonIsoEmBx.push_back(ne->bx());
      ...
      for(std::vector<UCTCandidate>::const_iterator it=l1NonIsoEmUCT->begin(); it!=l1NonIsoEmUCT->end(); it++) {

	UCTCandidate itr = (*it);
	double ET = itr.pt();
	if(itr.pt()>=63) ET=63;
	unsigned iEta=itr.getInt("rgnEta");
	unsigned rank = emScal->rank(ET) ;
	...
	gct_.EmEta.push_back(gctEmCand.regionId().ieta());
	gct_.EmPhi.push_back(gctEmCand.regionId().iphi());
	gct_.EmRnk.push_back(gctEmCand.rank());
	gct_.EmBx.push_back(gctEmCand.bx());
	gct_.EmIsoVal.push_back(itr.getFloat("relativeIsolationEG"));

      }

      gct_.EmSize = l1NonIsoEmUCT->size();
}
